  ...
  const confirmPrint = () => {
    ...
    for (let page = 0; page < totalPages; page++) {
      printableContent += `<div style="display:grid;grid-template-columns:repeat(${Math.min(
        ...
        2
      )},1fr);gap:10px;padding:10px;${page < totalPages - 1 ? "page-break-after:always;" : ""
        }">`;
      ...
      for (let i = 0; i < stickersOnThisPage; i++) {
        printableContent += `
          ...
          productToPrint.price
            ...
            .slice(-6)
            .toUpperCase()}</strong></span><span>Type: <strong>${productToPrint.category?.name || "N/A"
          }</strong></span></div>
            <div style="font-weight:bold;text-align:center;margin:4px 0;">${productToPrint.name
          }</div>
            <div style="display:flex;justify-content:space-between;font-size:12px;"><span>Size: <strong>${productToPrint.size?.join(", ") || "N/A"
          }</strong></span><span>Color: <strong>${productToPrint.color || "N/A"
          }</strong></span></div>
            ${productToPrint.fabric
            ? `<div style="font-size:12px;">Fabric: <strong>${productToPrint.fabric}</strong></div>`
            : ""
          }
            <div style="text-align:center;margin-top:5px;">${productToPrint.barcode
                ...
                <div className="flex justify-between items-start mb-2">
                  <div className="flex gap-1">
                    <button
                      ...
                      className={`p-1.5 rounded-full ${product.stock <= 0 ? "bg-gray-200" : "bg-primary-50"
                        }`}
                    >
                  ...
                  <button
                    ...
                    className={`p-1.5 rounded-full ${product.homeVisibility ? "bg-purple-100" : "bg-gray-100"
                      }`}
                  >
                    <Home size={16} />
                  </button>
              ...
              <div className="p-6">
                ...
                <div className="flex gap-3">
                  <button
                    ...
                    className={`flex-1 py-2 px-4 rounded text-white ${selectedProduct.stock <= 0 ? "bg-gray-400" : "bg-primary-600"
                      }`}
                  >
            ...
            <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
              <div className="p-6">
                ...
                <div>
                  ...
                  {/* Upload area */}
                  <label
                    className={`flex items-center justify-center w-full h-32 px-4 transition bg-white border-2 border-gray-300 border-dashed rounded-md cursor-pointer hover:border-primary-500 ${(editingProduct.images?.length || 0) + newImages.length >= 5
                    ...
                    <div className="flex flex-col items-center space-y-2">
                      ...
                      <span className="text-xs text-gray-500">
                        {(editingProduct.images?.length || 0) + newImages.length >=
                          5
                          ? "Maximum 5 images"
                          : `Up to 5 images (${(editingProduct.images?.length || 0) +
                          newImages.length
                          }/5)`}
                      </span>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setEditFormData({ ...editFormData, fabric: e.target.value })
                        }
                        className="w-full border rounded px-3 py-2"
                      />
                    </div>
