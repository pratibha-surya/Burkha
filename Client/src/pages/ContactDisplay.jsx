...
const ContactDisplay = () => {
  ...
  const fetchEnquiries = async () => {
    try {
      const response = await fetch('https://backend-20ar.onrender.com/contact/allcontact');
      if (!response.ok) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(`https://backend-20ar.onrender.com/contact/alldelete/${id}`, {
        method: 'DELETE',
      });
